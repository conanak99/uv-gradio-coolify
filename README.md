...
- **Batch edits**: The Edit tab accepts up to 5 input images per submission (uploaded images plus the optional image URL combined). Every selected model runs on every image in parallel, and results are captioned per image.
...
| `NANO_GPT_KEY` | — | NanoGPT API key used by Seedream/WAN generation and Seedream/WAN/P-Image edit models. |
